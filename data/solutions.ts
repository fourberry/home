...
export const fbSolutions: FbSolution[] = [
    ...
    },
    {
        ...
        desc: 'SMS·알림톡·이메일·푸시 발송을 공통 API로 연동하는 메시징 플랫폼입니다. 프로젝트별 채널 설정과 발송 이력을 관리하고, 큐와 워커로 발송 요청 접수와 전송 처리를 분리합니다.',
        feats: [
            { t: '통합 채널', d: 'SMS · LMS · MMS · 알림톡 · 이메일 · 푸시' },
            { t: '공통 API', d: '설정한 채널을 공통 규격으로 호출' },
            { t: '발송 처리', d: '큐·워커로 요청 접수와 전송 분리' },
            { t: '발송 관리', d: '프로젝트별 채널 설정·발송 이력' },
        ],
        seoTitle: '알림톡·SMS 통합 발송 API 솔루션 LIME (라임)',
        seoDescription:
            'SMS·LMS·MMS·카카오 알림톡·이메일·푸시 발송을 공통 API로 연동하는 라임. 프로젝트별 채널 설정과 발송 이력을 관리합니다. 채널별 준비 사항과 발송량에 맞는 운영 구성을 확인하세요.',
        ...
        sections: [
            {
                h: '공통 API로 메시지 발송 연동',
                body: 'SMS·LMS·MMS, 카카오 알림톡, 이메일, 푸시 발송을 공통 API로 연동합니다. 호출하는 서비스가 채널별 발송 업체의 API를 각각 구현하는 작업을 줄일 수 있습니다. 사용할 채널에 맞춰 발신 정보와 메시지 내용을 구성해야 하며, 채널 추가 시 필요한 설정과 연동 변경 범위는 별도로 확인합니다.',
            },
            {
                h: '큐와 워커로 요청 접수와 전송을 분리합니다',
                body: '발송 요청을 큐에 넣고 워커(발송 작업을 처리하는 별도 프로세스)가 실제 전송을 수행합니다. 호출한 서비스가 전송 완료까지 기다리는 부담을 줄이는 구조입니다. 실제 처리량과 대기 시간은 서버 자원, 워커 구성, 발송 업체의 제한에 따라 달라집니다. 요청 접수와 수신자에게 전달되는 시점은 구분해야 합니다.',
            },
            {
        ...
        faq: [
            {
                q: '어떤 채널을 지원하나요?',
                a: 'SMS·LMS·MMS 문자, 카카오 알림톡, 이메일, 앱 푸시 발송을 공통 API로 연동합니다. 사용할 채널에 맞는 발신 정보와 메시지 항목을 구성하며, 필요한 발송 서비스 연결과 준비 사항은 도입 전에 확인합니다.',
            },
            {
                q: '대량으로 보내면 서비스가 느려지지 않나요?',
                a: '큐와 워커로 요청 접수와 실제 전송을 분리해 전송 완료를 기다리는 부담을 줄입니다. 다만 서버 자원과 구성, 요청량에 따라 지연이 발생할 수 있습니다. 예상 최대 발송량과 발송 업체의 제한을 기준으로 운영 용량과 대기 시간을 확인해야 합니다.',
            },
            {
            ...
            {
                q: '발송 요청이 접수되면 수신자에게 전달된 것인가요?',
                a: '요청 접수와 실제 전송·수신은 구분됩니다. 발송 이력을 통해 처리 결과를 확인하고, 최종 수신 여부를 어디까지 확인할 수 있는지는 채널과 발송 업체가 제공하는 결과 정보를 기준으로 검토해야 합니다.',
            },
            {
                q: '도입 상담 전에 무엇을 준비하면 되나요?',
                a: '사용할 채널, 예상 발송량과 집중 시간대, 기존 발송 서비스, 연동할 시스템을 알려주세요. 설치 위치와 운영 담당 주체, 발송 실패 시 처리, 유지보수 범위와 채널별 이용료의 포함 여부를 확인할 수 있도록 요구사항을 정리하는 데 도움이 됩니다.',
            },
        ],
    ...
    },
    {
        ...
        desc: 'Playwright 기반 브라우저로 웹사이트의 로그인·조회·파일 다운로드를 자동화하는 솔루션입니다. 대상 사이트별 수집 모듈과 전용 관리 앱을 통해 반복 작업을 실행하고, 수집한 데이터를 OMS·SCM 등에 연결합니다.',
        feats: [
            { t: '실제 브라우저 기반', d: 'Playwright로 웹 화면 조작·수집' },
            { t: '자동 수집', d: '검색결과·상품정보·파일 추출' },
            { t: '프로세스 자동화', d: '로그인부터 다운로드까지' },
            { t: '시스템 연동', d: 'OMS · SCM 데이터 파이프라인' },
        ],
        seoTitle: '웹 스크래핑 · 업무 자동화 솔루션 MUSCAT (머스캣)',
        seoDescription: '브라우저 기반 웹 스크래핑·업무 자동화 솔루션 머스캣. 로그인·조회·파일 다운로드와 OMS·SCM 연동을 구성합니다. 대상 사이트의 인증 방식, 화면 변경과 운영 조건을 확인하세요.',
        headline: '반복하는 웹 업무를 자동화합니다.\n웹 스크래핑 솔루션 MUSCAT',
        category: 'BusinessApplication',
        problems: [
        ...
        sections: [
            ...
            {
                h: '대상 사이트별 모듈로 변경 범위를 나눕니다',
                body: '대상 사이트별 수집 로직을 모듈로 분리해 화면 개편 시 수정 범위를 좁힐 수 있습니다. 화면과 로그인 절차가 바뀌면 해당 모듈을 수정하고 다시 검증해야 합니다. 공통 실행 환경이나 여러 사이트가 함께 사용하는 인증 체계의 변경은 다른 작업에도 영향을 줄 수 있으므로 운영 시 함께 점검합니다.',
            },
            {
        ...
        faq: [
            {
                ...
                a: '로그인 후 조회와 다운로드를 하나의 작업으로 구성할 수 있습니다. 다만 추가 인증이나 수동 확인이 필요한 경우 자동화 범위가 달라집니다. 대상 사이트의 실제 인증 절차와 세션 만료 시 동작을 확인한 뒤 적용 가능 범위를 정해야 합니다.',
            },
            {
                q: '대상 사이트 화면이 바뀌면 어떻게 되나요?',
                a: '사이트별 모듈을 중심으로 변경된 화면과 처리 절차를 수정하고 재검증합니다. 공통 실행 환경이나 인증 체계도 바뀌었다면 다른 수집 작업에 미치는 영향을 함께 확인해야 합니다.',
            },
            {
            ...
            {
                q: '수집 속도와 실행 주기는 어떻게 정하나요?',
                a: '대상 사이트의 응답 속도와 접근 제한, 수집할 데이터 양, 로그인 절차와 실행 환경을 기준으로 확인합니다. 실제 작업 흐름을 검증한 뒤 실행 주기와 동시 작업 수를 정해야 하며, 모든 사이트에 동일한 처리 속도를 보장하는 방식은 아닙니다.',
            },
            {
                q: '도입 상담 전에 무엇을 준비하면 되나요?',
                a: '대상 사이트와 로그인 방식, 반복하는 작업 순서, 필요한 데이터·파일, 실행 주기와 연동할 시스템을 정리해 주세요. 실행 환경, 실패 시 확인·재실행 방식, 화면 변경에 따른 유지보수 범위를 함께 검토하는 데 도움이 됩니다.',
            },
        ],
